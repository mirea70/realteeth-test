...
class OutboxPublishTxFacadeTest {
        ...
        void completePublished_success() {
                ...
                given(imageJobPersistenceOutport.markPublishedDirectly(
                                eq(new com.realteeth.imagejob.model.ImageJobId(10L)), any(LocalDateTime.class)))
                                .willReturn(true);
                ...
                then(imageJobPersistenceOutport).should()
                                .markPublishedDirectly(eq(new com.realteeth.imagejob.model.ImageJobId(10L)),
                                                timeCaptor.capture());
        ...
        @Test
        @DisplayName("아웃박스는 성공했지만 이미지 작업 상태 변경에 실패하더라도 true를 반환한다 (이미 다른 상태일 수 있음)")
        void completePublished_success_evenWhenImageJobMarkFailed() {
                // given
                OutboxEvent event = OutboxEvent.fromOutside(
                                1L,
                                DomainType.IMAGE_JOB,
                ...
                given(outboxPersistenceOutport.markPublishedDirectly(1L)).willReturn(true);
                given(imageJobPersistenceOutport.markPublishedDirectly(
                                eq(new com.realteeth.imagejob.model.ImageJobId(30L)), any(LocalDateTime.class)))
                                .willReturn(false);
                ...
                then(outboxPersistenceOutport).should().markPublishedDirectly(1L);
                then(imageJobPersistenceOutport).should()
                                .markPublishedDirectly(eq(new com.realteeth.imagejob.model.ImageJobId(30L)),
                                                any(LocalDateTime.class));
        }

        @Test
        @DisplayName("POLL 이벤트의 경우 이미지 작업 상태를 변경하지 않고 true를 반환한다")
        void completePublished_success_withPollEvent() {
                // given
                OutboxEvent event = OutboxEvent.fromOutside(
                                1L,
                                DomainType.IMAGE_JOB,
